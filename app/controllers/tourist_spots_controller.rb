class TouristSpotsController < ApplicationController
  ...
  def create
    itinerary = Itinerary.find(params[:itinerary_id])
    tourist_spot_params = params.require(:tourist_spot).permit(:title, :comment, :location,
      :activity_date, :spot_type, :value)
    tourist_spot = TouristSpot.new(tourist_spot_params)
    itinerary.activities << tourist_spot
    redirect_to trip_plan_itinerary_path(tourist_spot.itinerary.trip_plan.id, tourist_spot.itinerary.id)
  end
end
